...
public class DeliveryPoint implements Comparable<DeliveryPoint> {
	...
	@ApiObjectField(name = "id", description = "" +
		"Generated automatically by database", required = true)
	private Long id;
	@Column(nullable = false)
	@ApiObjectField(name = "name", description = "Represents the name of the point", required = true)
	private String name;
	@OneToMany(cascade = CascadeType.ALL, fetch = FetchType.EAGER, orphanRemoval = true)
	@ApiObjectField(name = "adjacencies", description = "Represents the adjacent paths of a point", required = true)
	private List<DeliveryPath> adjacencies = new LinkedList<DeliveryPath>();
	@OneToOne
	@JoinColumn(name = "previous_point_reference")
	...
	@ApiObjectField(name = "name", description = "Represents the minimum distance between the current point and the next")
